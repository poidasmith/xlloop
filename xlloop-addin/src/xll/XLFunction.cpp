...
bool XLFunction::Initialize(JNIEnv* env)
{
	...
	const char* ft = JNI::CallJavaStringMethod(env, mFunctionClass, mFuncObj, "getName");
	if(ft == NULL) {
		return false;
	}
	mFunctionText = ft;

	const char* cat = JNI::CallJavaStringMethod(env, mFunctionClass, mFuncObj, "getCategory");
	if(cat == NULL) {
		return false;
	}
	mCategory = cat;

	const char* ht = JNI::CallJavaStringMethod(env, mFunctionClass, mFuncObj, "getHelpText");
	if(ht == NULL) {
		mFunctionHelp = "";
	} else {
		mFunctionHelp = ht;
	...
	for(int i = 0; i < arrSize; i++) {
		const char* argName = JNI::CallJavaStringMethod(env, argClass, env->GetObjectArrayElement(arr, i), "getName");
		std::string argNameStr = argName == NULL ? "" : argName;
		const char* argHelp = JNI::CallJavaStringMethod(env, argClass, env->GetObjectArrayElement(arr, i), "getHelpText");
