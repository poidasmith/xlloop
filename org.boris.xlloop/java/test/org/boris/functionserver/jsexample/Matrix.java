 ...
 * takes care of that by automatically allocating arrays for every index that is
 * accessed. What's more, the Matrix constructor takes a integer argument that
 * specifies the dimension of the Matrix. If m is a Matrix with dimension 3,
...
public class Matrix implements Scriptable {
    ...
    public Matrix(int dimension) {
        if (dimension <= 0) {
            throw Context
                    .reportRuntimeError("Dimension of Matrix must be greater than zero");
        }
        dim = dimension;
        v = new Vector();
    }
     ...
     * <p>
     * Handles the "dim" property and returns NOT_FOUND for all other names.
     * 
     ...
     * <p>
     * Look up the element in the associated vector and return it if it exists.
     ...
     * 
     * This method shouldn't even be called since we define all properties as
     * PERMANENT.
     */
     ...
     * instanceof operator.
     * 
     * We mimick the normal JavaScript instanceof semantics, returning true if
     * <code>this</code> appears in <code>value</code>'s prototype chain.
     */
